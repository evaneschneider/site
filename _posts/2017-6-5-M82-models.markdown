...
* A gas disk mass of 0.25 * total disk mass, and gas disk scale length of twice the stellar disk scale length. These numbers should perhaps be adjusted. Greco (2012) estimates a gas fraction of 40%.


The above parameters lead to a central surface density of $$\sim100 M_\odot$$/pc, which is consistent with M82's observed central surface density. M82 also contains a central stellar bar, which considerably affects the shape of the stellar rotation curve in the inner kpc. Below is my fit to the stellar rotation curve of Greco (2012), using only the disk and halo components given above:
...
These simulations are adiabatic, using an adiabtic index of 5/3. In order to prevent temperatures in the disk from going below $$10^3$$ K in the outer regions, I had to set the characteristic EOS temperature for the disk gas to $$2\times10^{5}$$ K, which is pretty high and leads to a pretty puffy disk. The characteristic temperature of the halo gas at the cooling radius is $$10^6$$ K.
